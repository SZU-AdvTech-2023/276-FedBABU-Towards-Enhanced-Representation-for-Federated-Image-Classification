...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        elif model_str == "conv4": # non-convex
            if "Cifar100" in args.dataset:
                args.model = CNNCifar(num_classes=args.num_classes).to(args.device)
            elif "Cifar10" in args.dataset:
                args.model = FedAvgCNN(in_features=3, num_classes=args.num_classes, dim=1600).to(args.device)
        elif model_str == "mobile": # non-convex
            if "Cifar100" in args.dataset:
                args.model = MobileNetCifar(num_classes=args.num_classes).to(args.device)
            elif "Cifar10" in args.dataset:
                args.model = FedAvgCNN(in_features=3, num_classes=args.num_classes, dim=1600).to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('-algo', "--algorithm", type=str, default="FedBABU")
